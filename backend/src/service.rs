...
use ryu_egress::{GuardedFetchPolicy, GuardedRequest, GuardedResponse};

use crate::{
    burst, canon, cluster, extract, feed,
    models::{
        Article, ArticleQuery, Brief, BriefItem, BriefSource, BriefTrigger, HeadlineSnapshot,
        NewArticle, SnapshotItem, Source, Topic, TopicBurst,
    },
    query, rank, simhash,
    state::{
        AppState, BRIEF_MODEL_PREF_KEY, EVENT_BRIEF_READY, EVENT_STORY_DEVELOPING,
        EVENT_TOPIC_BREAKING, OUTBOUND_CALL_TIMEOUT_MS,
    },
    store::NewsStore,
    text,
};
...
/// Maximum bytes accepted from one feed response. The shared egress primitive
/// enforces this while reading the response; the check in [`fetch_feed_with`]
/// keeps the News boundary explicit if that response type ever gains another
/// constructor.
pub const MAX_FEED_BODY_BYTES: usize = 5 * 1024 * 1024;

/// Keep the redirect behavior of the previous `reqwest` client while screening
/// every target through the shared egress primitive before the next request.
pub const MAX_FEED_REDIRECT_HOPS: usize = 10;
...
async fn ingest_source(state: &AppState, source: &Source, now: i64) -> Result<IngestReport> {
    ...
    let mut conditional_headers = Vec::with_capacity(2);
    if let Some(etag) = source.etag.as_deref() {
        conditional_headers.push(("if-none-match".to_owned(), etag.to_owned()));
    }
    if let Some(modified) = source.last_modified.as_deref() {
        conditional_headers.push(("if-modified-since".to_owned(), modified.to_owned()));
    }
    let response = fetch_feed(&source.feed_url, &conditional_headers).await?;
    ...
    let status = reqwest::StatusCode::from_u16(response.status)
        .map_err(|error| anyhow!("the source returned invalid HTTP status: {error}"))?;
    ...
    for item in parsed.items {
        ...
        let duplicate_of =
            find_duplicate(&state.store, &source.workspace_id, fingerprint, now).await;
        let new = NewArticle {
        ...
        if let Some(article) = state
            .store
            .insert_article(&source.workspace_id, &new, now)
            .await?
        {
...
/// Fetch one feed through the shared SSRF guard, following redirects one hop at a
/// time so every new host is resolved, checked and pinned before it is contacted.
async fn fetch_feed(url: &str, headers: &[(String, String)]) -> Result<GuardedResponse> {
    let policy = feed_fetch_policy();
    fetch_feed_with(url, headers, |request| {
        ryu_egress::guarded_request(request, policy)
    })
    .await
}

/// The redirect loop is separate from the concrete egress call so its compatibility
/// behavior is testable without contacting an external server. Production always
/// supplies [`ryu_egress::guarded_request`], which applies the SSRF policy on each
/// invocation.
async fn fetch_feed_with<F, Fut>(
    url: &str,
    headers: &[(String, String)],
    request: F,
) -> Result<GuardedResponse>
where
    F: FnMut(GuardedRequest) -> Fut,
    Fut: Future<Output = std::result::Result<GuardedResponse, String>>,
{
    fetch_feed_with_timeout(
        url,
        headers,
        Duration::from_millis(OUTBOUND_CALL_TIMEOUT_MS),
        request,
    )
    .await
}

async fn fetch_feed_with_timeout<F, Fut>(
    url: &str,
    headers: &[(String, String)],
    deadline: Duration,
    mut request: F,
    ...
    Fut: Future<Output = std::result::Result<GuardedResponse, String>>,
{
    tokio::time::timeout(deadline, async move {
        let mut current = reqwest::Url::parse(url.trim())
            .map_err(|error| anyhow!("invalid feed URL: {error}"))?;

        for hop in 0..=MAX_FEED_REDIRECT_HOPS {
            let response = request(GuardedRequest {
                method: "GET".to_owned(),
            ...
            if response.body.len() > MAX_FEED_BODY_BYTES {
                return Err(anyhow!(
                    "feed response body exceeds {MAX_FEED_BODY_BYTES} bytes"
                ));
            }

            if !is_feed_redirect(response.status) {
                return Ok(response);
            }

            let Some(location) = header(&response.headers, "location") else {
                // Match reqwest's behavior for a redirect without a usable Location:
                // return the response so the caller reports its non-success status.
                return Ok(response);
            };
            if hop == MAX_FEED_REDIRECT_HOPS {
                return Err(anyhow!(
                    "too many redirects (more than {MAX_FEED_REDIRECT_HOPS})"
                ));
            }
            current = current
                .join(&location)
                .map_err(|error| anyhow!("invalid redirect target: {error}"))?;
        }

        unreachable!("the bounded redirect loop always returns")
    })
    .await
    .map_err(|_| anyhow!("feed request timed out after {} ms", deadline.as_millis()))?
}

fn feed_fetch_policy() -> GuardedFetchPolicy {
    GuardedFetchPolicy {
        allow_http: true,
        max_body_bytes: MAX_FEED_BODY_BYTES as u64,
        max_redirect_hops: MAX_FEED_REDIRECT_HOPS,
        timeout: Duration::from_millis(OUTBOUND_CALL_TIMEOUT_MS),
    }
}

fn is_feed_redirect(status: u16) -> bool {
...
pub async fn cluster_pending(state: &AppState, now: i64) -> Result<(usize, usize)> {
    ...
    for workspace in workspaces {
        ...
        for article in pending {
            ...
            let features =
                cluster::features(&article.title, article.content.as_deref().unwrap_or(""));
            let stories = state
                .store
            ...
            if let burst::Verdict::Burst(stats) = verdict {
                ...
                state
                    ...
                    .emit_with_notify(
                        ...
                        // A watched topic breaking is genuinely time-sensitive.
                        Some(
                            ryu_app_events::NotifyHint::info(
                                format!("“{}” is breaking", topic.name),
                                Some(format!("{} stories in the last hour", stats.count)),
                            )
                            .with_level("warning"),
                        ),
                    )
...
async fn raise_developing_stories(state: &AppState) -> Result<()> {
    for workspace in state.store.list_workspaces().await? {
        for story in state
            .store
            .followed_stories_that_grew(&workspace.id)
            .await?
        {
            state
                .events
...
pub async fn generate_brief(state: &AppState, trigger: BriefTrigger, now: i64) -> Result<Brief> {
    ...
    state
        ...
        .emit_with_notify(
            ...
            // A scheduled brief is the ideal daily digest notification: one row per
            // morning, on a schedule the user chose.
            Some(
                ryu_app_events::NotifyHint::info(
                    "Your news brief is ready",
                    Some(format!("{} stories in this brief", brief.items.len())),
                )
                .with_level("success"),
            ),
        )
...
mod tests {
    ...
    #[tokio::test]
    async fn feed_fetch_preserves_conditionals_across_relative_redirects() {
        let conditional = vec![
            ("if-none-match".to_owned(), "\"wire-v1\"".to_owned()),
            (
                "if-modified-since".to_owned(),
                "Wed, 12 Aug 2026 08:00:00 GMT".to_owned(),
            ),
        ];
        let mut seen = Vec::new();
        let response = fetch_feed_with(
            "https://news.example.test/feeds/main.xml",
            &conditional,
            |request| {
                ...
                let response = if first {
                    GuardedResponse {
                        status: 302,
                        headers: vec![("Location".to_owned(), "/feeds/final.xml".to_owned())],
                        body: Vec::new(),
                    }
                } else {
                    GuardedResponse {
                        status: 200,
                        headers: vec![("etag".to_owned(), "\"wire-v2\"".to_owned())],
                        body: b"<rss/>".to_vec(),
                    }
                };
        ...
        .await
        .expect("the final feed response should be returned");

        assert_eq!(seen.len(), 2);
        assert_eq!(seen[0].method, "GET");
        assert_eq!(seen[1].method, "GET");
        assert_eq!(seen[0].headers, conditional);
        assert_eq!(seen[1].headers, conditional);
        assert_eq!(seen[1].url, "https://news.example.test/feeds/final.xml");
        assert_eq!(response.status, 200);
        assert_eq!(
            header(&response.headers, "etag").as_deref(),
            Some("\"wire-v2\"")
        );
        assert_eq!(response.body, b"<rss/>".to_vec());
    }

    #[tokio::test]
    async fn feed_fetch_returns_not_modified_without_following() {
        let mut calls = 0;
        let response = fetch_feed_with("https://news.example.test/feeds/main.xml", &[], |_| {
            calls += 1;
            async {
                Ok(GuardedResponse {
        ...
        .await
        .expect("304 is a terminal response");

        assert_eq!(calls, 1);
        assert_eq!(response.status, reqwest::StatusCode::NOT_MODIFIED.as_u16());
    }

    #[tokio::test]
    async fn feed_fetch_uses_one_deadline_for_the_whole_redirect_chain() {
        let mut calls = 0;
        let error = fetch_feed_with_timeout(
            "https://news.example.test/feeds/main.xml",
            &[],
            ...
            |request| {
                ...
                async move {
                    // Each hop is individually under the 100ms limit, but the
                    // chain is not. A per-hop timeout would incorrectly succeed.
        ...
        .await
        .expect_err("the aggregate deadline must stop a slow redirect chain");
    ...
    #[tokio::test]
    async fn feed_fetch_rejects_private_redirect_destinations_before_connecting() {
        let policy = feed_fetch_policy();
        let mut calls = 0;
        let error = fetch_feed_with("https://news.example.test/feeds/main.xml", &[], |request| {
            let first = calls == 0;
            calls += 1;
            async move {
                if first {
                    Ok(GuardedResponse {
                        ...
                        headers: vec![(
                            "location".to_owned(),
                            "http://127.0.0.1:8080/private.xml".to_owned(),
                        )],
                        body: Vec::new(),
                    })
                } else {
                    ryu_egress::guarded_request(request, policy).await
                }
            }
        })
        .await
    ...
    #[tokio::test]
    async fn feed_fetch_rejects_private_initial_destinations_before_connecting() {
        let error = fetch_feed("http://127.0.0.1:8080/feed.xml", &[])
            .await
            .expect_err("loopback feed URLs must be refused");

        assert!(error.to_string().contains("private/loopback"), "{error}");
    }

    #[tokio::test]
    async fn feed_fetch_rejects_responses_above_the_body_ceiling() {
        assert_eq!(
            feed_fetch_policy().max_body_bytes,
            MAX_FEED_BODY_BYTES as u64
        );
        let response = GuardedResponse {
            status: 200,
            headers: Vec::new(),
            body: vec![0; MAX_FEED_BODY_BYTES + 1],
        };
        let error = fetch_feed_with("https://news.example.test/feeds/main.xml", &[], |_| {
            let response = response.clone();
            async move { Ok(response) }
        })
        .await
        .expect_err("an oversized response must not reach the parser");

        assert!(
            error
                .to_string()
                .contains("feed response body exceeds 5242880 bytes"),
            "{error}"
        );
    }
