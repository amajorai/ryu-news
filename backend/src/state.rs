...
/// The one HTTP client shape this process uses for loopback host callbacks.
///
...
/// `reqwest::Client` owns a connection pool, so a client per host callback would
/// re-do TLS for every call. The external feed path uses
/// [`ryu_egress::guarded_request`] instead: that primitive resolves and pins each
/// destination before it opens a connection. Keeping redirects disabled here makes
/// an accidental use of this loopback client fail closed rather than turning it into
/// a second unguarded fetch path. Falls back to the default client if the builder
...
pub fn build_http_client() -> reqwest::Client {
    reqwest::Client::builder()
        ...
        // Host callbacks are loopback-only. External feed requests use the guarded
        // per-hop client in `service.rs`, where every redirect target is screened.
...
pub struct AppState {
    pub store: NewsStore,
    /// Shared client for the loopback Host bridge. External feed requests use the
    /// per-hop `ryu-egress` client rather than this pooled client.
