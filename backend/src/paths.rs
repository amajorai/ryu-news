...
//! `apps-store/quests/backend/src/paths.rs`, `apps-store/teams/backend/src/paths.rs`,
//! `apps-store/mail/backend/src/paths.rs` and
//! `apps-store/social/backend/src/paths.rs`).
//!
//! The sidecar MUST resolve the SAME data dir Core uses so it opens the SAME
//! `news.db` the node uses. The load-bearing rule is `RYU_DIR`-env-first:
//! Core/Kernel passes `RYU_DIR` to the sidecar at spawn, guaranteeing co-location.
//! The pointer-file read + `RYU_PROFILE` suffix are replicated for faithfulness in
//! the headless case, but env-first + default is what actually guarantees the
//! shared path.
//!
//! Copied rather than re-derived on purpose: a hand-rolled `~/.ryu` would drop the
//! `RYU_PROFILE` suffix, and the sidecar would then open a DIFFERENT `news.db` than
//! the node under `RYU_PROFILE=dev` — a feed that silently collects into a file
//! nothing reads.
