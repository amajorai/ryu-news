...
mod tests {
    ...
    fn entity_decoding_table() {
        let cases: &[(&str, &str, &str)] = &[
            ...
            (
                "hex numeric, either case",
                "caf&#xE9; caf&#Xe9;",
                "café café",
            ),
            ("uppercase named form", "a &AMP; b", "a & b"),
            (
                "unknown entity is left alone",
                "Q&A and &hearts;",
                "Q&A and &hearts;",
            ),
            ("a bare ampersand survives", "Tom & Jerry", "Tom & Jerry"),
            (
                "no semicolon in the window",
                "&averyverylongthing",
                "&averyverylongthing",
            ),
...
<rss version="2.0" xmlns:content="http://purl.org/rss/1.0/modules/content/"
  ...
  <channel>
    ...
    fn rss_two_point_zero_reads_every_field_it_should() {
        ...
        assert_eq!(first.content.as_deref(), Some("Full text with a link."));
  ...
  <entry>
    ...
    fn malformed_input_table() {
        let cases: &[(&str, &str, FeedError)] = &[
            ...
            ("whitespace only", "   \n\t ", FeedError::Empty),
            (
                "an HTML page instead of a feed",
                "<!DOCTYPE html><html><body>Login</body></html>",
                FeedError::UnknownFormat,
            ),
            ("plain prose", "not xml at all", FeedError::UnknownFormat),
            (
                "angle-bracket soup with no element",
                "<<<>>>",
                FeedError::UnknownFormat,
            ),
            (
                "JSON that is not a feed",
                "{\"hello\":\"world\"}",
                FeedError::UnknownFormat,
            ),
            (
                "truncated JSON",
                "{\"items\": [",
                FeedError::Json(String::new()),
            ),
            (
                "a feed cut inside its first tag",
                "<rss><channel><item><title attr=\"unterminated",
                FeedError::Malformed(String::new()),
            ),
        ];
    ...
    fn date_parsing_table() {
        let cases: &[(&str, &str, Option<i64>)] = &[
            (
                ...
                "Mon, 10 Aug 2026 08:55:00 +0000",
                Some(1_786_352_100_000),
            ),
            (
                "RFC 2822 with GMT",
                "Mon, 10 Aug 2026 08:55:00 GMT",
                Some(1_786_352_100_000),
            ),
            (
                "RFC 2822 with a trailing comment",
                "Mon, 10 Aug 2026 08:55:00 +0000 (UTC)",
                Some(1_786_352_100_000),
            ),
            (
                "RFC 3339 with Z",
    ...
    fn attributes_parse_through_quotes_spaces_and_missing_values() {
        assert_eq!(
            attr("href=\"https://x.test/a\" rel=\"alternate\"", "href"),
            Some("https://x.test/a")
        );
        assert_eq!(
            attr("href='https://x.test/b'", "HREF"),
            Some("https://x.test/b")
        );
        assert_eq!(
            attr("href = https://x.test/c type=text/html", "href"),
            Some("https://x.test/c")
        );
    ...
    #[test]
    fn parse_feed_handles_a_large_comment_and_declaration_run_in_html_fields() {
        let mut noise = String::new();
        for _ in 0..25_000 {
            noise.push_str("<!-- ignored --><?ignored?><!ignored>");
        }
        let body = format!(
            "<rss><channel><title>Wire</title><item><title>Visible item</title>\
             <link>https://example.test/story</link><description>{noise}Visible summary</description>\
             </item></channel></rss>"
        );

        let feed = parse_feed(&body, NOW).expect("the hostile HTML field should still parse");
        assert_eq!(feed.items.len(), 1);
        assert_eq!(feed.items[0].title, "Visible item");
        assert_eq!(feed.items[0].summary.as_deref(), Some("Visible summary"));
    }
}
