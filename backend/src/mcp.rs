...
//! - **Tool names are registered BARE.** Core forms `{server}__{tool}` from the
//!   `mcp_servers` key, so registering `news__search` here yields `news__news__search`.
...
mod tests {
    ...
    fn no_tool_name_is_self_prefixed() {
        // Core forms `{server}__{tool}`, so a self-prefixed name here becomes
        // `news__news__search` and no caller can ever reach it.
        for tool in tools().as_array().expect("an array") {
            let name = tool["name"].as_str().expect("a name");
            assert!(!name.contains("__"), "'{name}' is self-prefixed");
            assert!(!name.starts_with("news"), "'{name}' repeats the server name");
        }
    }
    ...
    fn every_tool_has_an_object_schema_and_a_real_description() {
        for tool in tools().as_array().expect("an array") {
            ...
            assert_eq!(tool["inputSchema"]["type"], "object", "'{name}' must take an object");
        }
    }
