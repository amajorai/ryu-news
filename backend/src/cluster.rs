...
mod tests {
    ...
    fn an_unrelated_article_opens_its_own_story() {
        ...
        assert_eq!(assign(&c, &[candidate("s1", &a, NOW)], NOW), Assignment::Open);
    }
    ...
    fn the_centroid_freezes_so_a_story_cannot_drift() {
        let a = features(REUTERS.0, REUTERS.1);
        let c = features(UNRELATED.0, UNRELATED.1);
        let (frozen, _) = fold_centroid(
            &a.shingles,
            &a.entities,
            &c,
            CENTROID_FREEZE_MEMBERS,
        );
        assert_eq!(frozen, a.shingles, "centroid must not absorb past the freeze");

        let (grown, _) = fold_centroid(&a.shingles, &a.entities, &c, 1);
        assert!(grown.len() > a.shingles.len(), "centroid must grow before it");
    }
    ...
    fn entities_keep_accruing_after_the_centroid_freezes() {
        ...
        let c = features(UNRELATED.0, UNRELATED.1);
        let (_, entities) =
            fold_centroid(&a.shingles, &a.entities, &c, CENTROID_FREEZE_MEMBERS);
    ...
    fn a_sentence_boundary_breaks_an_entity_run() {
        ...
        assert!(!found.iter().any(|e| e.contains(' ') && e.contains("berlin")), "got {found:?}");
    }
