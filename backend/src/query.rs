...
pub enum Node {
    ...
    Phrase { field: Field, words: Vec<String> },
}
...
fn lex(input: &str) -> Result<Vec<Spanned>, QueryError> {
    ...
    while i < chars.len() {
        ...
        match ch {
            '(' => {
                out.push(Spanned { tok: Tok::LParen, column });
                i += 1;
            }
            ')' => {
                out.push(Spanned { tok: Tok::RParen, column });
                i += 1;
            }
            ':' => {
                out.push(Spanned { tok: Tok::Colon, column });
                i += 1;
            }
            '-' => {
                // A leading `-` is negation; a `-` inside a word is part of it
                // ("covid-19"), which the word branch below handles.
                out.push(Spanned { tok: Tok::Not, column });
                i += 1;
            }
            '"' => {
                ...
                out.push(Spanned { tok: Tok::Quoted(buf), column });
            }
            _ => {
                let start = i;
                while i < chars.len()
...
impl Parser {
    ...
    fn err(&self, message: impl Into<String>) -> QueryError {
        QueryError { message: message.into(), column: self.column() }
    }
    ...
    fn parse_not(&mut self, depth: usize) -> Result<Node, QueryError> {
        if matches!(self.peek(), Some(Tok::Not)) {
            ...
            return Ok(Node::Not { node: Box::new(node) });
        }
...
fn term_node(field: Field, raw: &str, column: usize) -> Result<Node, QueryError> {
    ...
    match tokens.len() {
        ...
        1 => Ok(Node::Term { field, word: tokens.remove(0) }),
        _ => Ok(Node::Phrase { field, words: tokens }),
    }
}
...
fn phrase_node(field: Field, raw: &str, column: usize) -> Result<Node, QueryError> {
    ...
    if words.len() == 1 {
        let mut words = words;
        return Ok(Node::Term { field, word: words.remove(0) });
    }
...
pub fn parse(input: &str) -> Result<Node, QueryError> {
    ...
    if toks.is_empty() {
        return Err(QueryError { message: "query is empty".into(), column: 1 });
    }
    let mut parser = Parser { toks, pos: 0, end_column };
