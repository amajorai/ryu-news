...
use crate::{
    ...
    models::{
        now_ms, ArticleMark, ArticleQuery, BriefTrigger, NewSource, NewsSettings, SourceKind,
    },
    query, service,
    state::AppState,
};
...
#[openapi(
    ...
    components(schemas(FollowBody, MarkBody, NewsSettings, OpmlBody, SourceBody, TopicBody,))
)]
struct NewsApiDoc;
...
async fn list_sources(State(state): State<AppState>) -> ApiResult<Json<Value>> {
    ...
    Ok(Json(
        json!({ "sources": state.store.list_sources(&workspace_id).await? }),
    ))
}
    ...
    let articles = state
        ...
        .list_articles(
            ...
            &ArticleQuery {
                ...
                since: params
                    .since_hours
                    .map(|h| now - h.clamp(1, 24 * 365) * 3_600_000),
...
async fn mark_archived(
    ...
    mark(
        &state,
        &id,
        ArticleMark::Archived,
        body.map_or(true, |b| b.on),
    )
    .await
}
...
async fn follow_story(
    ...
    require_hit(
        state.store.set_story_followed(&id, followed).await?,
        "story",
    )
}
...
async fn list_topics(State(state): State<AppState>) -> ApiResult<Json<Value>> {
    let workspace_id = workspace(&state).await?;
    Ok(Json(
        json!({ "topics": state.store.list_topics(&workspace_id).await? }),
    ))
}
...
fn compile(raw: &str) -> ApiResult<(Value, query::Node)> {
    let node = query::parse(raw)
        .map_err(|err| ApiError::BadRequest(format!("{} (column {})", err.message, err.column)))?;
...
async fn topic_matches(
    ...
    let articles = state
        .store
        .topic_match_articles(
            &id,
            from,
            now,
            params.limit.unwrap_or(100).clamp(1, MAX_LIMIT),
        )
        .await?;
    let last_burst = state.store.last_burst(&id).await?;
    Ok(Json(
        json!({ "articles": articles, "last_burst": last_burst }),
    ))
}
...
async fn list_briefs(
    ...
    let briefs = state
        ...
        .list_briefs(
            &workspace_id,
            params.limit.unwrap_or(30).clamp(1, MAX_LIMIT),
        )
        .await?;
