...
impl<'a> Scanner<'a> {
    ...
    ///
    /// Skip paths use a loop rather than calling back into this method. A server can
    /// return an arbitrary run of comments or declarations inside feed HTML, and
    /// one stack frame per skipped token would turn that input into a process abort.
...
mod tests {
    ...
    fn entity_decoding_table() {
        let cases: &[(&str, &str, &str)] = &[
            ...
            (
                "no semicolon within the window",
                "&averyverylongthing",
                "&averyverylongthing",
            ),
            (
                "out-of-range codepoint is left alone",
                "&#1114112;",
                "&#1114112;",
            ),
    ...
    fn a_document_with_no_qualifying_subtree_falls_back_to_the_whole_text() {
        assert_eq!(
            extract_main_text("<p>Too short to be an article.</p>"),
